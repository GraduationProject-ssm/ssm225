...
# ssm225在线房屋租赁和电子签约系统的设计与实现+vue

![picture](https://raw.githubusercontent.com/GraduationProject-springboot/.github/main/img/wx.png)

### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1gn8XeNE2J?p=24)
...
随着计算机技术的发展以及计算机网络的逐渐普及，互联网成为人们查找信息的重要场所，二十一世纪是信息的时代，所以信息的交换和信息流通显得特别重要。因此，开发合适的在线房屋租赁和电子签约系统的设计与实现成为企业必然要走的一步棋。开发合适的在线房屋租赁和电子签约系统的设计与实现，可以方便管理人员对在线房屋租赁和电子签约系统的设计与实现的管理，提高信息管理工作效率及查询效率，有利于更好的为用户提供服务。

1.2研究目的

随着互联网技术的快速发展，网络时代的到来，网络信息也将会改变当今社会。各行各业在日常企业经营管理等方面也在慢慢的向规范化和网络化趋势汇合。在线房屋租赁和电子签约系统的设计与实现的信息化程度体现在将互联网与信息技术应用于经营与管理，以现代化工具代替传统手工作业。无疑，使用网络信息化管理使信息管理更先进、更高效、更科学，信息交流更迅速。

房屋租赁如果还用之前的只有线下形式的去实行，已经很落伍了，这样会导致了效率低下。而且，时间一长的话，积累下来的数据信息不容易保存，对于查询、更新还有维护会带来不少问题。对于数据交接也存在很大的隐患。如果采用电子化的存储方式就会带来很大的改善，而且给用户的查询带来了很大便利，因此设计一个在线房屋租赁和电子签约系统的设计与实现刻不容缓，能够提高企业在信息技术方面的展示水平。
## 1.3系统设计思想
...
# 3系统分析
## 3.1可行性分析
通过对本在线房屋租赁和电子签约系统的设计与实现实行的目的初步调查和分析，提出可行性方案并对其一一进行论证。我们在这里主要从技术可行性、经济可行性、操作可行性等方面进行分析。
### 3.1.1技术可行性
本在线房屋租赁和电子签约系统的设计与实现采用JAVA作为开发语言，SSM框架，是基于WEB平台的B/S架构系统。
...
因此，在线房屋租赁和电子签约系统的设计与实现在开发技术上具有很高可行性，且开发人员掌握了一定的开发技术，所以此系统的开发技术具有可行性。
### 3.1.2经济可行性
本在线房屋租赁和电子签约系统的设计与实现采用的软件都是开源的，这样能够削减很多的精力和资源，降低开发成本。同时对计算机的配置要求也极低，即使是淘汰下来的计算机也能够满足需要，因此，本系统在经济上是完全具有可行性的，所以在经济上是十分可行的。
### 3.1.3操作可行性
本在线房屋租赁和电子签约系统的设计与实现的界面简单易操作，用户只要平时有在用过电脑，都能进行访问和操作。本系统具有易操作、易管理、交互性好的特点，在操作上是非常简单的，因此在操作上具有很高的可行性。

综上所述，此系统开发目标已明确，在技术、经济和操作方面都具有很高的可行性，并且投入少、功能完善、管理方便，因此系统的开发是完全可行的。
## 3.2系统性能分析
### 3.2.1 系统安全性
此在线房屋租赁和电子签约系统的设计与实现要严格控制管理权限，具体要求如下：

（1）要想对在线房屋租赁和电子签约系统的设计与实现进行管理，首先要依靠用户名和密码在系统中登陆，无权限的用户不可以通过任何方式登录系统和对系统的任何信息和数据进行查看，这样可以保证系统的安全可靠性和准确性。
...
本系统是基于B/S架构的网站系统，设计的经纪人功能结构图如下图所示：

![](/md/blog.005.png)

图4-3 经纪人功能结构图

本系统是基于B/S架构的网站系统，设计的租户功能结构图如下图所示：
...
（1）合同文件信息的实体属性图如图4.5所示：

![](/md/blog.007.png)

图4.5合同文件信息实体属性图

（2）房子户型信息实体属性图如图4.6所示：

![](/md/blog.008.png)

图4.6房子户型信息实体属性图

（3）房源类型信息实体属性图如图4.7所示：
...
### 4.3.2数据库设计表
在线房屋租赁和电子签约系统的设计与实现需要后台数据库，下面介绍数据库中的各个表的详细信息：
...
|diqu|varchar(200)|是|NULL|地区|
|fangyuanleixing|varchar(200)|是|NULL|房源类型|
|chengjiaojiage|int(11)|是|NULL|成交价格|
...
|fangyuanleixing|varchar(200)|是|NULL|房源类型|
|fangzihuxing|varchar(200)|是|NULL|房子户型|
|fangyuanweizhi|varchar(200)|是|NULL|房源位置|
|zhuangtai|varchar(200)|是|NULL|状态|
|fangyuanmianji|varchar(200)|是|NULL|房源面积|
|zujin|int(11)|是|NULL|租金|
|zulinqixian|varchar(200)|是|NULL|租赁期限|
|fangyuanjieshao|longtext|是|NULL|房源介绍|
|fangdongxingming|varchar(200)|是|NULL|房东姓名|
|zhanghao|varchar(200)|是|NULL|账号|
|xingming|varchar(200)|是|NULL|姓名|
|shouji|varchar(200)|是|NULL|手机|
|hetongwenjian|longtext|是|NULL|合同文件|
|chengjiaoshijian|date|是|NULL|成交时间|
|fangyuantupian|varchar(200)|是|NULL|房源图片|
|chengjiaojia|int(11)|是|NULL|成交价|
...
|fangyuanmingcheng|varchar(200)|是|NULL|房源名称|
|fangyuanweizhi|varchar(200)|是|NULL|房源位置|
|zuhuzhanghao|varchar(200)|是|NULL|租户账号|
|zuhuxingming|varchar(200)|是|NULL|租户姓名|
|zhanghao|varchar(200)|是|NULL|账号|
|xingming|varchar(200)|是|NULL|姓名|
|zujin|int(11)|是|NULL|租金|
|zulinqixian|varchar(200)|是|NULL|租赁期限|
|hetongwenjian|varchar(200)|是|NULL|合同文件|
...
|zuhuxingming|varchar(200)|是|NULL|租户姓名|
|hetongwenjian|varchar(200)|是|NULL|合同文件|
|jiaoyishijian|datetime|是|NULL|交易时间|
...
|fangyuantupian|varchar(200)|是|NULL|房源图片|
|fangyuanweizhi|varchar(200)|否||房源位置|
|zujin|int(11)|否||租金|
|zulinqixian|varchar(200)|是|NULL|租赁期限|
|zuhuzhanghao|varchar(200)|是|NULL|租户账号|
|zuhuxingming|varchar(200)|是|NULL|租户姓名|
|tuizuriqi|date|是|NULL|退租日期|
|tuizubeizhu|longtext|是|NULL|退租备注|
|zhanghao|varchar(200)|是|NULL|账号|
|xingming|varchar(200)|是|NULL|姓名|
|shenfenzheng|varchar(200)|是|NULL|身份证|
|sfsh|varchar(200)|是|否|是否审核|
...
|gongzuotaidu|int(11)|是|NULL|工作态度|
|yewujineng|int(11)|是|NULL|业务技能|
|gongzuojixiao|int(11)|是|NULL|工作绩效|
|jiangchengjiajianfen|int(11)|是|NULL|奖惩加减分|
|zongdefen|varchar(200)|是|NULL|总得分|
|pingjiadengji|varchar(200)|是|NULL|评价等级|
|pingjiayuefen|varchar(200)|是|NULL|评价月份|
表4.16 租户
...
|id (主键)|bigint(20)|否||主键|
|addtime|timestamp|否|CURRENT\_TIMESTAMP|创建时间|
|zuhuzhanghao|varchar(200)|否||租户账号|
|mima|varchar(200)|否||密码|
|zuhuxingming|varchar(200)|否||租户姓名|
|xingbie|varchar(200)|是|NULL|性别|
...
## 5.1 管理员模块的实现
### 5.1.1 租户管理管理
在线房屋租赁和电子签约系统的设计与实现的系统管理员可以管理租户管理信息，可以对租户管理信息添加修改删除操作。具体界面的展示如图5.1所示。

![](/md/blog.011.png)

图5.1 租户管理界面
### 5.1.2 房源类型管理
系统管理员可以对房源类型信息进行添加，修改，删除操作。具体界面如图5.2所示。
...
### 5.2.2 合同文件管理
经纪人登录后，可以对合同文件进行添加修改删除操作。界面如下图所示：
...
## 5.3 租户模块的实现
### 5.3.2 交易订单支付
租户登录后，在后台交易订单里面可以对订单进行支付操作。界面如下图所示：

![](/md/blog.015.png)

图5.5交易订单支付界面
### 5.3.2 我的收藏管理
租户登录后可以在我的收藏管理里面删除自己收藏的信息，也可以查看自己收藏的信息。界面如下图所示：
